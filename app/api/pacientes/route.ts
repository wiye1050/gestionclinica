...
export async function GET(request: Request) {
  ...
  try {
    let query = db.collection('pacientes').orderBy('apellidos').orderBy('nombre');
    if (estado) {
      query = query.where('estado', '==', estado);
    }

    if (cursorId) {
      const cursorSnap = await db.collection('pacientes').doc(cursorId).get();
    ...
    const safeMap = (docSnap: typeof docs[number]) => {
