...
export async function createProfesional(input: ProfesionalInput, actor: { userId: string; userEmail?: string }) {
...
export async function deleteProfesional(profesionalId: string, actor: { userId: string; userEmail?: string }) {
  const db = assertAdminDb();
  const docRef = db.collection('profesionales').doc(profesionalId);
  const snapshot = await docRef.get();
  if (!snapshot.exists) {
